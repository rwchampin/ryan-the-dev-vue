...
function init() {
  ...
  const buildSceneMesh = () => {
    ...
    const textTexture = THREE.ImageUtils.loadTexture('public/three-assets/logo.png');
  ...
  gsap.fromTo(
    camera.position,
    { y: window.innerHeight / , x: window.innerWidth / 2 },
    {
      y: 0,
      scrollTrigger: {
...
function render() {
  mesh.rotation.x += 0.005 * Math.PI * 2;
  mesh.rotation.y -= 0.05 * Math.PI * 2;
  cubeSineDriver += 0.01;
