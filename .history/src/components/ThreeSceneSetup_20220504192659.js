...
export class ThreeSceneSetup {
  constructor() {
    this.this.container = document.createElement('canvas');
    this.renderer = new THREE.WebGLRenderer({
      antialias: true,
      stencil: false
    });
    this.renderer.setSize(this.this.container.offsetWidth, this.this.container.offsetHeight, false);
    this.renderer.setPixelRatio(Math.max(1.5, window.devicePixelRatio));

    this.stats = new Stats();
    document.body.appendChild(this.stats.dom);

    this.this.container.append(this.renderer.domElement);
