...
export default {
  ...
  mounted() {
    this.setCursorAnimation(this);
  },
  components: {
    AnimationUtils
  },
  methods: {
    setCursorAnimation(that) {
      const canvas = document.getElementById('canvasB'),
        context = canvas.getContext('2d'),
        colorPallete = ['#B2B1CF', '#82aaff', '#000000'];
      ...
      loop();
      function loop() {
        that.$refs.animationUtils.statsBegin();
        ...
        removeBall();
        that.$refs.animationUtils.statsEnd();
...
<template>
  <div>
    ...
    <AnimationUtils ref="animationUtils" />
    <svg style="background-image: radial-gradient( circle 651px at 7% 22.8%, rgba(232,249,255,1) 10.5%, rgba(229,199,206,1) 51.3%, rgba(219,199,229,1) 97.6% );" xmlns="http://www.w3.org/2000/svg" version="1.1">
      <defs>
        <filter id="goo">
